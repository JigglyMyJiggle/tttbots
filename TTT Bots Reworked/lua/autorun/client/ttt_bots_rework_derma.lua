...
local drawEyeView=false
hook.Add( "HUDPaint", "3d_camera_example2", function()
    if info["dbg"] == nil or info["dbg"] == 0 then return end
	cam.Start3D()

    if (drawEyeView) then
		for id, ply in pairs( player.GetBots() ) do
            if ply:Health() > 0 then
                if IsValid(ply:GetEyeTrace().Entity) then
                    if ply:GetEyeTrace().Entity:IsPlayer() then
                        render.DrawLine(ply:EyePos(), ply:GetEyeTrace().HitPos, Color(255, 0, 0,125))
                    else
                        render.DrawLine(ply:EyePos(), ply:GetEyeTrace().HitPos, Color(0, 255, 0,125))
                    end
                else
                    render.DrawLine(ply:EyePos(), ply:GetEyeTrace().HitPos, Color(0, 255, 0,125))
                end
    ...
    if info.statuses != nil then
        for i,v in pairs(info.statuses) do -- statuses.drawlines = {}
            if v.bot:Health() > 0 then
                if v.drawlines != nil then
                    for z,x in pairs(v.drawlines) do
                        render.DrawLine(v.bot:EyePos(), x.pos, x.color) -- {pos=Vector,color=Color}
                    end
                end
            end
        end
...
hook.Add("HUDPaint", "DisplayDataUIBotsTTT", function()
    if info["dbg"] == nil or info["dbg"] == 0 then return end
    if info.statuses == nil or info.rndstrt == nil then return end
    surface.SetDrawColor( 0, 0, 0, 200 )
	surface.DrawRect( 0, surface.ScreenHeight()/6, surface.ScreenWidth()/6, 10000 --[[surface.ScreenWidth()/10 + (#info.statuses-1)*30 ///\\\  2*surface.ScreenHeight()/3]] )
    
    for i,v in pairs(info.statuses) do
        surface.SetFont("DebugFixed")

        local hppct = v.bot:Health()/v.bot:GetMaxHealth()

        if (v.hppct > 0.8) then
            surface.SetDrawColor( 0, 255, 0, 100 )
        elseif (v.hppct > 0.6) then
            surface.SetDrawColor( 50, 200, 0, 100 )
        elseif v.hppct > 0.4 then
            surface.SetDrawColor( 100, 100, 0, 100 )
        elseif v.hppct > 0.2 then
            surface.SetDrawColor( 200, 50, 0, 100 )
        else
            surface.SetDrawColor( 255, 0, 0, 100 )
        end
	    surface.DrawRect( 0, (surface.ScreenWidth()/10 + (i-1)*50), hppct * (surface.ScreenWidth()/6), 50)


        surface.SetTextColor(255, 255, 255) -- draw name
        surface.SetTextPos(0, surface.ScreenWidth()/10 + (i-1)*50)
        surface.DrawText(v.name)

        if (info.rndstrt) then
            ...
            -- draw role
            surface.SetTextPos(120, surface.ScreenWidth()/10 + (i-1)*50)
            surface.DrawText("-- "..v.role)
        else
            surface.SetTextColor(100, 100, 100)
            surface.SetTextPos(120, surface.ScreenWidth()/10 + (i-1)*50)
            surface.DrawText("-- ".." unset")
        end


        surface.SetTextColor(0, 255, 0) -- draw status
        surface.SetTextPos(0, surface.ScreenWidth()/10 + (i-1)*50 + 12)
        surface.DrawText(v.status)
    end
end)